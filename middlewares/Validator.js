...
const validateSchema = function (schema, data) {
  ...
  if (validator.error) {
    validator.error = validator.error.details[0].message;
  }
  return validator;
};
...
const ProductValidator = (data) => {
  const schema = Joi.object({
    ...
    category: Joi.string().valid("officeSupplies", "kitchenAppliances", "computers", "furniture", "warehouses").required()
  });
  return validateSchema(schema, data);
};
...
const ReportValidator = (data) => {
  const schema = Joi.object({
    ...
    report: Joi.string().required().trim(),
  })
  return validateSchema(schema, data);
}
const DepartmentValidator = (data) => {
  const schema = Joi.object({
    name: Joi.string().required().trim(),
  })
  return validateSchema(schema, data);
