...
ProductController.create = async (req, res) => {
  const { name, code, rate, category } = req.body;
  const validator = ProductValidator({ name, code, rate, category });
  if (validator.error) {
  ...
  try {
    const { name, code, rate, category } = validator.value;
    const savedProduct = await new Product({ name, code, rate, category }).save();
    await new Inventory({
