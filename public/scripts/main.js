...
const fetchURI = {
  ...
  departments: `${hostAPI}/departments`,
};
...
const departmentInputQuery = document.querySelector('.department-input-query');
const departmentQueryGroup = document.querySelector('.department-input-query-group');
if (departmentInputQuery) {
  departmentInputQuery.addEventListener('input', () =>
    searchData(
      departmentInputQuery.value,
      fetchURI.departments,
      'department',
      'name',
      'name',
      departmentInputQuery,
      departmentQueryGroup
    )
  );
}
...
/*******************************
 * Get Department and Edit
 *******************************/
const departmentForm = document.querySelector('.departmentForm');
const departmentFormTitle = document.querySelector('.departmentForm-title');
const departmentFormName = document.querySelector('.departmentForm-name');
const departmentEditButtons = document.querySelectorAll('.departmentEditButton');

if (departmentEditButtons) {
  departmentEditButtons.forEach((btn) => {
    btn.addEventListener('click', async () => {
      const ItemID = btn.parentElement.getAttribute('data-item-id');
      const res = await fetch(`${hostAPI}/departments/${ItemID}`);
      const { name } = await res.json();
      departmentForm.setAttribute('action', `/departments/${ItemID}?_method=patch`);
      departmentFormTitle.innerText = 'Edit Department';
      departmentFormName.value = name;
      $('[data-dismiss="modal"]').on('click', function () {
        departmentForm.reset();
        departmentFormTitle.innerText = 'Add New Department';
        departmentFormAmount.removeAttribute('disabled');
        departmentFormPaid.removeAttribute('disabled');
        departmentForm.setAttribute('action', '/departments');
      });
    });
  });
}
