...
CustomerController.create = async (req, res) => {
  const { name, phone, department, amount, paid } = req.body;
  const validator = CustomerValidator({ name, phone, department });
  if (validator.error) {
  ...
  try {
    const { name, phone, department } = validator.value;
    const dbDepartment = await Department.findOne({
      name: department,
    });
    customer = new Customer({ name: name, phone: phone, department: dbDepartment._id });
    await customer.save();
